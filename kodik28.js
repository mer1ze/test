...
export default class extends Extension {
  ...
  async detail(id) {
    ...
    return {
      ...
      cover: anime.image?.original ? `${this.shikimoriDomain}${item.image.original}` : "",
  ...
  async watch(url) {
    ...
    if (typeof resHtml !== "string") {
      throw new Error("Не удалось загрузить страницу плеера");
    }

    // Обработка iframe вложений
    ...
    // 1. Попытка извлечь scriptParams объекта (современный формат Kodik)
    let domain = "";
    let d_sign = "";
    let pd = "";
    let pd_sign = "";
    ...
    const scriptParamsMatch = resHtml.match(/scriptParams\s*=\s*(\{[\s\S]*?\});/);
    if (scriptParamsMatch && scriptParamsMatch[1]) {
      try {
        const rawJson = scriptParamsMatch[1]
    ...
    // 2. Фолбэк на прямой поиск объявлений переменных в JS
    if (!d_sign) {
      d_sign = (resHtml.match(/var\s+d_sign\s*=\s*["']([^"']+)["']/) || [])[1] ||
               (resHtml.match(/["']d_sign["']\s*:\s*["']([^"']+)["']/) || [])[1] ||
               (resHtml.match(/d_sign:\s*["']([^"']+)["']/) || [])[1];
    }

    if (!domain) {
      domain = (resHtml.match(/var\s+domain\s*=\s*["']([^"']+)["']/) || [])[1] ||
               (resHtml.match(/["']domain["']\s*:\s*["']([^"']+)["']/) || [])[1] ||
               (resHtml.match(/domain:\s*["']([^"']+)["']/) || [])[1];
    }

    if (!pd) {
      pd = (resHtml.match(/var\s+pd\s*=\s*["']([^"']+)["']/) || [])[1] ||
           (resHtml.match(/["']pd["']\s*:\s*["']([^"']+)["']/) || [])[1] || "";
    }

    if (!pd_sign) {
      pd_sign = (resHtml.match(/var\s+pd_sign\s*=\s*["']([^"']+)["']/) || [])[1] ||
                (resHtml.match(/["']pd_sign["']\s*:\s*["']([^"']+)["']/) || [])[1] || "";
    }

    if (!ref) {
      ref = (resHtml.match(/var\s+ref\s*=\s*["']([^"']+)["']/) || [])[1] ||
            (resHtml.match(/["']ref["']\s*:\s*["']([^"']+)["']/) || [])[1] || "";
    }

    // 3. Извлечение домена из самого URL, если он не прописан в скриптах
    if (!domain) {
      try {
    ...
    if (!d_sign) {
      throw new Error("Параметры плеера недоступны. Возможно, запрашиваемый релиз заблокирован в Kodik.");
    }
    ...
    if (!gtaRes || !gtaRes.links) {
      throw new Error("Kodik не вернул ссылки на поток");
    }
