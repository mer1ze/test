...
export default class extends Extension {
  ...
  async watch(url) {
    ...
    // Поиск scriptParams или urlParams
    const paramsMatch = resHtml.match(/(?:scriptParams|urlParams)\s*=\s*(\{[\s\S]*?\});/);
    ...
    if (!d_sign) {
      throw new Error("Не удалось извлечь подпись плеера (d_sign). Возможно, запрашиваемый релиз заблокирован.");
    }
