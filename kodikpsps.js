...
export default class extends Extension {
  ...
  async fetchApi(url, options = {}) {
    // Чтобы ядро Miru не пыталось открыть длинный URL как файл, 
    // передаем короткий фиксированный маршрут-заглушку в первый аргумент.
    const safePath = (typeof url === 'string' && url.length > 200) ? "https://kodik-api.com" : url;

    return await this.request(safePath, {
      headers: {
        "Miru-Url": url,
  ...
  async watch(urlStr) {
    ...
    const pdSign = urlParamsJson.pd_sign || extract(/pd_sign["']?\s*[:=]\s*["']([^"']+)["']/i);
    const refSign = urlParamsJson.ref_sign || extract(/ref_sign["']?\s*[:=]\s*["']([^"']+)["']/i);
    const dParam = urlParamsJson.d || extract(/var\s+domain\s*=\s*["']([^"']+)["']/i) || "kodikplayer.com";
    const pdParam = urlParamsJson.pd || "kodikplayer.com";

    let videoType = extract(/vInfo\.type\s*=\s*["']([^"']+)["']/i) || extract(/var\s+type\s*=\s*["']([^"']+)["']/i) || "seria";
    ...
    if (!dSign || !videoHash || !videoId) {
      throw new Error(`Не удалось спарсить параметры видео. Hash: ${videoHash}, ID: ${videoId}, Sign: ${dSign}`);
    }
    ...
    const postDataObj = {
      ...
      pd: pdParam,
      pd_sign: pdSign,
      ref: urlParamsJson.ref ? decodeURIComponent(urlParamsJson.ref) : "",
      ref_sign: refSign,
      bad_user: "false",
      cdn_is_working: "true",
      type: videoType,
    ...
    // Вызываем через короткий домен, чтобы обойти ограничение длины пути в движке
