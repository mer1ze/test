...
export default class extends Extension {
  ...
  async watch(url) {
    ...
    // Если получили обёртку с iframe, проваливаемся внутрь целевого плеера
    ...
    // Извлекаем параметры из переменных или JSON-объектов страницы
    let domain = (resHtml.match(/var domain = "(.+?)";/) || [])[1] || (resHtml.match(/"domain":\s*"(.+?)"/) || [])[1];
    let d_sign = (resHtml.match(/var d_sign = "(.+?)";/) || [])[1] || (resHtml.match(/"d_sign":\s*"(.+?)"/) || [])[1];
    ...
    if (!domain || !d_sign) {
      // Резервный вызов API Kodik, если сгенерированная ссылка не смогла распарситься
      const shikimoriIdMatch = url.match(/shikimori_id=(\d+)/);
      const episodeMatch = url.match(/episode=(\d+)/);
      
      if (shikimoriIdMatch) {
        const shId = shikimoriIdMatch[1];
        const epNum = episodeMatch ? episodeMatch[1] : "1";
        
        const apiRes = await this.request("", {
          headers: {
            "Miru-Url": `https://kodikapi.com/v2/search?shikimori_id=${shId}&token=${this.kodikToken}&with_episodes=true`,
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)",
          },
        });

        if (apiRes && apiRes.results && apiRes.results[0]) {
          const directLink = apiRes.results[0].seasons ? apiRes.results[0].seasons[1]?.episodes[epNum] : apiRes.results[0].link;
          if (directLink && directLink !== url) {
            return this.watch(directLink);
          }
        }
      }
      throw new Error("Не удалось извлечь параметры плеера Kodik");
    }
