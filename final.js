...
export default class extends Extension {
  ...
  decodeUrl(url) {
    ...
    try {
      // Сначала декодируем из Base64, затем применяем ROT13 (или наоборот в зависимости от упаковки)
  ...
  async watch(urlStr) {
    ...
    let parsedDomain = "kodikplayer.com";
    try {
      const parsedUrlObj = new URL(cleanUrl);
      parsedDomain = parsedUrlObj.hostname;
    } catch (e) {}

    const dParam = urlParamsJson.d || extract(/var\s+domain\s*=\s*["']([^"']+)["']/i) || parsedDomain;
    const pdParam = urlParamsJson.pd || parsedDomain;
    ...
    const domainsToTry = [dParam, parsedDomain, "kodikplayer.com", "kodik.info"];
    let ftorRes = null;

    for (const dom of domainsToTry) {
      if (!dom) continue;
      try {
        ftorRes = await this.fetchApi(`https://${dom}/ftor`, {
          method: "POST",
          headers: {
        ...
        if (ftorRes && ftorRes.links) break;
      } catch (err) {}
    }
