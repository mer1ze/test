...
export default class extends Extension {
  ...
  async watch(urlStr) {
    ...
    // 1. Ищем JSON-конфиг urlParams (основной источник новых подписей)
    let urlParamsJson = {};
    ...
    // 2. Ищем параметры самого видео (Kodik использует vInfo или обычные var)
    ...
    // 3. Фоллбэк: берем hash и id из самого URL (железобетонный метод)
    ...
    if (ftorRes && ftorRes.links) {
      ...
      const bestQuality = qualities[qualities.length - 1]; // Берем лучшее качество
