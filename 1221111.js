...
export default class extends Extension {
  ...
  async fetchApi(url, options = {}) {
    // Безопасная проверка: если в url передали длинную строку, не пускаем ее в файловый метод
    const targetUrl = (typeof url === 'string' && url.length < 500) ? url : "";
    
    return await this.request("", {
      headers: {
        "Miru-Url": targetUrl || url,
