...
export default class extends Extension {
  ...
  async detail(id) {
    // Исправлено: жестко разделяем домен и путь, чтобы исключить дублирование
    const requestUrl = `${this.shikimoriDomain}/api/animes/${id}`;
      ...
      for (const release of kodikRes.results) {
        ...
        if (release.seasons) {
          ...
          for (let i = 1; i <= release.last_episode; i++) {
            urlsList.push({
              name: `Серия ${i}`,
              url: `${release.link}|${id}\vert{}${i}`,
            });
          }
        } else if (release.link) {
          urlsList.push({
            name: "Фильм / OVA",
            url: `${release.link}\vert{}${id}|1`,
          });
        }
