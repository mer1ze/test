...
export default class extends Extension {
  ...
  async watch(url) {
    ...
    playerUrl = playerUrl
      ...
      .replace("kodikapi.com", "kodikplayer.com")
    ...
    try {
      ...
      throw new Error("Ошибка подключения к Kodik");
    }
    ...
    const domain = (resHtml.match(/var domain = "(.+?)";/) || [])[1] || (resHtml.match(/"domain":\s*"(.+?)"/) || [])[1];
    const d_sign = (resHtml.match(/var d_sign = "(.+?)";/) || [])[1] || (resHtml.match(/"d_sign":\s*"(.+?)"/) || [])[1];
    const pd = (resHtml.match(/var pd = "(.+?)";/) || [])[1] || (resHtml.match(/"pd":\s*"(.+?)"/) || [])[1] || "";
    const pd_sign = (resHtml.match(/var pd_sign = "(.+?)";/) || [])[1] || (resHtml.match(/"pd_sign":\s*"(.+?)"/) || [])[1] || "";
    const ref = (resHtml.match(/var ref = "(.+?)";/) || [])[1] || (resHtml.match(/"ref":\s*"(.+?)"/) || [])[1] || "";

    if (!domain || !d_sign) {
      throw new Error("Плеер заблокирован провайдером");
    }
    ...
    if (!gtaRes || !gtaRes.links) {
      throw new Error("Ссылка на видеопоток не получена");
    }
