...
export default class extends Extension {
  ...
  // Запрос к API Kodik через рабочие домены (избегаем заблокированный kodikapi.com)
  ...
  // Главная страница: через Shikimori
  async latest(page) {
  ...
  // Загрузка детальной информации, озвучек и серий
  async detail(id) {
    ...
    if (kodikRes && kodikRes.results && kodikRes.results.length > 0) {
      for (const release of kodikRes.results) {
        const translationName = release.translation ? release.translation.title : "Стандартная озвучка";
        const urlsList = [];
  ...
  // Парсинг потока непосредственно из ссылки на плеер серии
  async watch(url) {
    ...
    if (!domainMatch || !dSignMatch) {
      throw new Error("Серия не найдена или заблокирована правообладателем");
    }
    ...
    if (!gtaRes || !gtaRes.links) {
      throw new Error("Не удалось извлечь видеоссылку");
    }
