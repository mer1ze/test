...
export default class extends Extension {
  ...
  async fetchApi(url, options = {}) {
    // Если переданная строка похожа на мусор/токен стрима (слишком длинная), не передаем ее в заголовки
    const isValidUrl = typeof url === 'string' && (url.startsWith('http://') || url.startsWith('https://') || url.startsWith('//'));
    const targetUrl = isValidUrl && url.length < 500 ? url : "";

    return await this.request("", {
      headers: {
        ...(targetUrl ? { "Miru-Url": targetUrl } : {}),
