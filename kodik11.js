...
export default class extends Extension {
  ...
  async watch(url) {
    ...
    if (typeof resHtml !== "string") {
      throw new Error("Не удалось прочитать страницу плеера");
    }

    // Извлекаем iframe, если исходная ссылка ведёт на фрейм-обертку
    ...
    if (!domain || !d_sign) {
      throw new Error("Не удалось получить видеопоток. Возможно, требуется включить proxy/VPN.");
    }
    ...
    if (!gtaRes || !gtaRes.links) {
      throw new Error("Плеер не вернул прямую ссылку на видео");
    }
